...
import { IJWTPayload, IPaginationOptions } from "../../types/common";
...
const createAppointment = async (user: IJWTPayload, payload: any) => {
    ...
    if (doctorData.appointmentFee < 60) {
        throw new Error("Appointment fee must be at least 60 BDT to process payment.");
    }
...
const initiatePaymentForAppointment = async (appointmentId: string, user: IJWTPayload) => {
    const patientData = await prisma.patient.findUniqueOrThrow({
        where: {
            email: user?.email
        }
...
const cancelUnpaidAppointments = async () => {
    ...
    await prisma.$transaction(async (tnx) => {
        // 1. Delete linked Prescriptions first
        ...
        // 3. Now you can safely delete the Appointments
        await tnx.appointment.deleteMany({
            where: {
                id: {
                    in: appointmentIdsToCancel
