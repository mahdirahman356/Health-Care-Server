...
const changePassword = async (user: any, payload: any) => {
    ...
    const hashedPassword: string = await bcrypt.hash(payload.newPassword, Number(config.bcrypt.salt_round));
...
const forgotPassword = async (payload: { email: string }) => {
    ...
    const resetPassToken = jwtHelper.generateToken(
        ...
        config.jwt.reset_token_expires as string
    )
...
const resetPassword = async (token: string | null, payload: { email?: string, password: string }, user?: { email: string }) => {
    ...
    if (token) {
        const decodedToken = jwtHelper.verifyToken(token, config.jwt.reset_token_secret as Secret)
    ...
    // hash password
    const password = await bcrypt.hash(payload.password, Number(config.bcrypt.salt_round));
